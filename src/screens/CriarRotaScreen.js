...
export default class CriarRotaScreen extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            quantidadeItem: 0,
            user: [],
            nome: '', data: '', hora: '', duracao: '', cidade: '', estado: '', qtdturistas: '',
            personalidade: '', descricao: '', saida: '', link: '', valor: '', avaliacao: '0'
        };
    }
    ...
    cadastrarRota = async () => {
        ...
        if (this.state.nome.length === 0 || this.state.data.length === 0
            || this.state.hora.length === 0 || this.state.duracao.length === 0
            || this.state.cidade.length === 0 || this.state.estado.length === 0
            || this.state.qtdturistas.length === 0 || this.state.personalidade.length === 0
            || this.state.descricao.length === 0 || this.state.saida.length === 0
            || this.state.link.length === 0 || this.state.valor.length === 0) {
            ...
            try {
                ...
                this.state.loading = false;
                this.setState({ error: 'Houve um problema ao cadastrar, verifique se todos campos foram preenchidos!' });
                Alert.alert('Erro', 'Houve um problema ao cadastrar, verifique se todos campos foram preenchidos!');
                console.log(_err);
            }
        }
    }
                ...
                <View
                    ...
                    <ScrollView>
                        ...
                        <Button
                            ...
                            title={<Text style={{ color: 'white' }}>Cadastrar Rota</Text>}>
                        </Button>
                    </ScrollView>
                </View>
            </View>
